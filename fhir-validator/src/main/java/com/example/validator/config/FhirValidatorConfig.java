...
public class FhirValidatorConfig {
   ...
   *  - A custom SNOMED validator module that calls Snowstorm directly
   *
   * Why a custom SNOMED module?
   *  - Snowstorm's FHIR API is used for validation of SNOMED CT codes.
   *  - By registering a separate custom validator module, we ensure clear validation context
   *    and explicit handling of SNOMED code validation independent of the main chain.
   *  - SNOMED code validation (system + code) is version-agnostic and works seamlessly with R5.
   */
  @Bean
