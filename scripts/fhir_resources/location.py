...
from scripts.enum_models import ConceptEnum, Locations
...
def build_hospitalized_location(hospitalized_in: ConceptEnum, admission_department: ConceptEnum) -> Location:
    """
    Build a FHIR Location resource from raw data.
    ...
    """
    loc = Location()
    loc.id = f"location-{hospitalized_in.id}"
    ...
    loc.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/hospitalized-location-profile"])
