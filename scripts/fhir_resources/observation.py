...
from fhir.resources.fhirtypes import InstantType, MetaType, ObservationComponentType, QuantityType, RangeType, TimingType
from fhir.resources.codeableconcept import CodeableConcept
from fhir.resources.extension import Extension
import pandas as pd
from enum_models import (
    AnaliticsCodes, AtrialFibrillationOrFlutter, BodySites, ConceptEnum, GlasgowComaScale, Laterality, ObservationMethods, ProcedureNotDoneReason, QualifierValue, RiskFactor, Symptomps, ThreeMonthContactMode, VitalSigns, MRsScore,
    AssessmentContext, FunctionalScore, MTiciScore, SpecificFinding,
    StrokeCircumstance, UnitofMeasurement, TimingMetricCodes
)
...
def build_observation_vital_signs(systolic_pressure: int | None, diastolic_pressure: int | None, patient_ref: str, encounter_ref: str, timing: AssessmentContext | None) -> Observation:
    ...
    observation = Observation(
        ...
        meta=MetaType(profile=["http://tecnomod-um.org/StructureDefinition/vital-sign-observation-profile"]),
    )
    observation.code = CodeableConcept(coding=[VitalSigns.TAKE_VS.to_coding()])
    component_list = []
...
def build_observation_mrs(patient_ref: str, encounter_ref: str, mrs_score: MRsScore | None, prestroke: bool | None = False , discharge: bool | None = False , threem: bool | None = False) -> Observation:
    ...
    return Observation(
        status="final",
        meta=MetaType(profile=["http://tecnomod-um.org/StructureDefinition/functional-score-observation-profile"]),
        subject=Reference(reference=patient_ref),
        encounter=Reference(reference=encounter_ref),
        code=code_mrs,
...
def build_observation_mtici_score(mtici_score: str, patient_ref: str, encounter_ref: str) -> Observation:
    """
    ...
    mtici_score_coding = MTiciScore.by_id(mtici_score).to_coding()
    code_value_mtici = CodeableConcept(coding=[mtici_score_coding])
    
    return Observation(
        status="final",
        subject=Reference(reference=patient_ref),
        meta=MetaType(profile=["http://tecnomod-um.org/StructureDefinition/specific-finding-observation-profile"]),
        code=code_mtici,
...
def build_observation_carotid_stenosis(patient_ref: str, encounter_ref: str, carotid_stenosis: bool | None, carotid_stenosis_level: str | None) -> Observation:
    """
    ...
    """
    specific_finding_coding = SpecificFinding.CAROTID_STENOSIS.to_coding()
    code_carotid_stenosis = CodeableConcept(coding=[specific_finding_coding])
    ...
    if carotid_stenosis is not None and carotid_stenosis_level is not None:
        stenosis_value_coding = QualifierValue.by_id(carotid_stenosis_level).to_coding()
        code_value_stenosis = CodeableConcept(coding=[stenosis_value_coding])
    
    return Observation(
        status="final",
        subject=Reference(reference=patient_ref),
        meta=MetaType(profile=["http://tecnomod-um.org/StructureDefinition/specific-finding-observation-profile"]),
        code=code_carotid_stenosis,
        valueCodeableConcept=code_value_stenosis,
        category=[code_category],
        encounter=Reference(reference=encounter_ref)
    )
...
def build_observation_Af_or_F(patient_ref: str, encounter_ref: str, atrial_fibrillation_or_flutter: AtrialFibrillationOrFlutter, hospital_timestamp:str |None = None) -> Observation:
    ...
    observation.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/specific-finding-observation-profile"])
...
def build_observation_smoking(raw: dict, patient_ref: str, encounter_ref: str) -> Observation:
    ...
    return Observation(
        status="final",
        subject=Reference(reference=patient_ref),
        meta=MetaType(profile=["http://tecnomod-um.org/StructureDefinition/base-stroke-observation"]),
        code=code_smoker,
        encounter=Reference(reference=encounter_ref),
    )
...
def build_stroke_circumstance_observation(patient_ref: str, encounter_ref: str, condition_ref: str, wake_up: bool = False, in_hosp: bool = False) -> Observation:
    ...
    return Observation(
        status="final",
        subject=Reference(reference=patient_ref),
        meta=MetaType(profile=["http://tecnomod-um.org/StructureDefinition/stroke-circumstance-observation-profile"]),
...
def build_timing_d2n_observation(door_to_needle_time: int, patient_ref: str, encounter_ref: str, procedure_ref: str) -> Observation:
    ...
    return Observation(
        ...
        subject=Reference(reference=patient_ref),
        encounter=Reference(reference=encounter_ref),
        meta=MetaType(profile=["http://tecnomod-um.org/StructureDefinition/timing-metric-observation-profile"]),
        code=code_timing,
        valueQuantity= QuantityType(
...
def build_observation_three_month_contact_mode(three_month_contact_mode: ThreeMonthContactMode, patient_ref: str, encounter_ref: str) -> Observation:
    ...
    return Observation(
        ...
        subject=Reference(reference=patient_ref),
        encounter=Reference(reference=encounter_ref),
        meta=MetaType(profile=["http://tecnomod-um.org/StructureDefinition/three-month-contact-mode-observation-profile"]),
...
def build_groing_to_reperfusion_observation(groin_to_reperfusion_time: int, patient_ref: str, encounter_ref: str, procedure_ref: str) -> Observation:
    """
...
def build_observation_glucose(glucose: float | None, patient_ref: str, encounter_ref: str, timing: AssessmentContext | None = None) -> Observation:
    ...
    observation.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/analitics-observation-profile"])
    ...
    category_code = CodeableConcept(coding=[category_coding])
    observation.category = [category_code]
    observation.valueQuantity = QuantityType(
...
def build_observation_glasgow_coma_scale(patient_ref: str, encounter_ref: str,gcs_score: int) -> Observation:
    ...
    category_code = CodeableConcept(coding=[category_coding])
    observation.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/glasgow-coma-scale-observation-profile"])
    observation.code = code_gcs
    observation.category = [category_code]
    observation.valueInteger = gcs_score
    observation.subject = Reference(reference=patient_ref)
    observation.encounter = Reference(reference=encounter_ref)
    if gcs_score > 3 and gcs_score <= 8:
        observation.interpretation = [CodeableConcept(coding=[QualifierValue.SEVERE.to_coding()])]
    elif gcs_score > 8 and gcs_score <= 12:
        observation.interpretation = [CodeableConcept(coding=[QualifierValue.MODERATE.to_coding()])]
    elif gcs_score > 12 and gcs_score <= 15:
        observation.interpretation = [CodeableConcept(coding=[QualifierValue.MILD.to_coding()])]
...
def build_observation_inr(patient_ref: str, encounter_ref: str, inr_value: int | None, inr_mode: str | None) -> Observation:
    ...
    if inr_mode is not None:
        if inr_mode == "not done" or inr_mode == "":
            ...
            observation.method = CodeableConcept(coding=[ObservationMethods.by_id(inr_mode).to_coding()])
...
def build_observation_perfusion(perfusion_found:bool, patient_ref: str, encounter_ref: str, perfusion_anterior : bool | None, perfusion_posterior : bool | None, perfusion_carotid: bool | None = None, perfusion_volume: int | None = None, hypoperfusion_volume: int | None = None) -> Observation:
    ...
    observation_core_volume = ObservationComponent(
        ...
        valueQuantity=QuantityType(value=Decimal(perfusion_volume) if perfusion_volume is not None else None, unit=UnitofMeasurement.ML.display, system=UnitofMeasurement.ML.system, code=UnitofMeasurement.ML.code)
    )
    ...
    observation_hypoperfusion_volume = ObservationComponent(
        ...
        valueQuantity=QuantityType(value=Decimal(hypoperfusion_volume) if hypoperfusion_volume is not None else None, unit=UnitofMeasurement.ML.display, system=UnitofMeasurement.ML.system, code=UnitofMeasurement.ML.code)
    )
    observation.component.append(observation_hypoperfusion_volume)

    return observation           

def build_observation_perfusion_volume(perfusion_volume: int, patient_ref: str, encounter_ref: str) -> Observation:
    """
    Build a FHIR Observation resource for perfusion volume.
    ...
    Args:
        ...
        encounter_ref: Reference to the Encounter resource
    Returns:
        Observation resource for perfusion volume
    """
    observation = Observation(
        code=CodeableConcept(),
        status="final"
    )
    perfusion_coding = SpecificFinding.PERFUSION_VOLUME.to_coding()
    code_perfusion = CodeableConcept(coding=[perfusion_coding])
    observation.code = code_perfusion
    category_coding = Coding(
        system="http://terminology.hl7.org/CodeSystem/observation-category",
        code="procedure",
        display="Procedure"
    ...
    category_code = CodeableConcept(coding=[category_coding])
    observation.category = [category_code]
    observation.valueQuantity = QuantityType(value=Decimal(perfusion_volume), unit=UnitofMeasurement.ML.display, system=UnitofMeasurement.ML.system, code=UnitofMeasurement.ML.code)
...
def build_observation_hypoperfusion_volume(hypoperfusion_volume: int, patient_ref: str, encounter_ref: str) -> Observation:
    """
    Build a FHIR Observation resource for hypoperfusion volume.
    ...
    Args:
        ...
        encounter_ref: Reference to the Encounter resource
    Returns:
        Observation resource for hypoperfusion volume
    """
    observation = Observation(
        code=CodeableConcept(),
        status="final"
    )
    hypoperfusion_coding = SpecificFinding.HYPOPERFUSION_VOLUME.to_coding()
    code_hypoperfusion = CodeableConcept(coding=[hypoperfusion_coding])
    observation.code = code_hypoperfusion
    category_coding = Coding(
        system="http://terminology.hl7.org/CodeSystem/observation-category",
        code="procedure",
        display="Procedure"
    ...
    category_code = CodeableConcept(coding=[category_coding])
    observation.category = [category_code]
    observation.valueQuantity = QuantityType(value=Decimal(hypoperfusion_volume), unit=UnitofMeasurement.ML.display, system=UnitofMeasurement.ML.system, code=UnitofMeasurement.ML.code)
...
def build_observation_perfusion_anterior(perfusion_anterior: bool, patient_ref: str, encounter_ref: str) -> Observation:
    """
    Build a FHIR Observation resource for anterior perfusion abnormality.
    ...
    Args:
        ...
        encounter_ref: Reference to the Encounter resource
    Returns:
        Observation resource for anterior perfusion abnormality
    """
    ...
    perfusion_coding = SpecificFinding.PERFUSION.to_coding()
    code_perfusion = CodeableConcept(coding=[perfusion_coding])
    observation.code = code_perfusion
    category_coding = Coding(
        system="http://terminology.hl7.org/CodeSystem/observation-category",
...
def build_observation_perfusion_carotid(perfusion_carotid: bool, patient_ref: str, encounter_ref: str) -> Observation:
    """
    Build a FHIR Observation resource for carotid perfusion abnormality.
    ...
    Args:
        ...
        encounter_ref: Reference to the Encounter resource
    Returns:
        Observation resource for carotid perfusion abnormality
    """
...
def build_observation_perfusion_medial(perfusion_medial: bool, patient_ref: str, encounter_ref: str) -> Observation:
    """
    Build a FHIR Observation resource for medial perfusion abnormality.
    
    Args:
        perfusion_medial: Boolean indicating if medial perfusion abnormality was found
        patient_ref: Reference to the Patient resource
        encounter_ref: Reference to the Encounter resource
    Returns:
        Observation resource for medial perfusion abnormality
    """
    ...
    observation.bodySite = CodeableConcept(coding=[Laterality.MEDIAL.to_coding()])
    observation.subject = Reference(reference=patient_ref)
    observation.encounter = Reference(reference=encounter_ref)

    return observation  

def build_observation_perfusion_posterior(perfusion_posterior: bool, patient_ref: str, encounter_ref: str) -> Observation:
    """
    Build a FHIR Observation resource for posterior perfusion abnormality.
    ...
    Args:
        ...
        encounter_ref: Reference to the Encounter resource
    Returns:
        Observation resource for posterior perfusion abnormality
    """
...
def build_observation_aspect_score(patient_ref: str, encounter_ref: str, aspect_score: int) -> Observation:
    ...
    observation.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/functional-score-observation-profile"])
    observation.subject = Reference(reference=patient_ref)
    observation.encounter = Reference(reference=encounter_ref)
...
def build_tia_clinical_symptomps_observation(patient_ref: str, encounter_ref: str, tia_symptom: Symptomps, tia_duration: str | None ) -> Observation:
    """
    ...
    if tia_duration == "<10 minutes":
        observation.valueQuantity = QuantityType(value=Decimal(10),comparator="<", unit=UnitofMeasurement.MINUTE.display, system=UnitofMeasurement.MINUTE.system, code=UnitofMeasurement.MINUTE.code)
    elif tia_duration == "10-59 minutes":
        observation.valueRange = RangeType(low=QuantityType(value=Decimal(10), unit=UnitofMeasurement.MINUTE.display, system=UnitofMeasurement.MINUTE.system, code=UnitofMeasurement.MINUTE.code), high=QuantityType(value=Decimal(59), unit=UnitofMeasurement.MINUTE.display, system=UnitofMeasurement.MINUTE.system, code=UnitofMeasurement.MINUTE.code))
    elif tia_duration == ">=60 minutes":
        observation.valueQuantity = QuantityType(value=Decimal(60),comparator=">=", unit=UnitofMeasurement.MINUTE.display, system=UnitofMeasurement.MINUTE.system, code=UnitofMeasurement.MINUTE.code)

    #observation.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/tia-clinical-symptoms-observation-profile"])
    observation.subject = Reference(reference=patient_ref)
    observation.encounter = Reference(reference=encounter_ref)
...
def build_observation_patient_ventilated(patient_ref: str, encounter_ref: str, ventilated: bool = False) -> Observation:
    ...
    #observation.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/patient-ventilated-observation-profile"])
    observation.subject = Reference(reference=patient_ref)
    observation.encounter = Reference(reference=encounter_ref)
    extension_list = []
    ...
    if post_treatment_findings_any is False:
        ...
        if post_treatment_hemorrhagic_transformation:
            if hemorrhagic_transformation_type is not None:
                observation_hemorrhagic_transformation.component = [ObservationComponentType(code=CodeableConcept(coding=[hemorrhagic_transformation_type.to_coding()]), valueBoolean=True)]
...
def build_observation_fever(patient_ref: str, encounter_ref: str, observation_measurement_ref: list[str] | None , fever: bool) -> Observation:
    ...
    observation.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/fever-observation-profile"])
...
def build_observation_hyperglycemia_measurement_checks(patient_ref: str, encounter_ref: str, day_1_hyperglycemia: int | None , day_2_hyperglycemia: int | None , day_3_hyperglycemia: int| None ) -> list[Observation]:
    ...
    if day_1_hyperglycemia is not None:
        observation_day_1_hyperglycemia = Observation(
            ...
            subject=Reference(reference=patient_ref),
            encounter=Reference(reference=encounter_ref),
            meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/hyperglycemia-observation-profile"]),
...
def build_observation_hyperglycemia_day_2(patient_ref: str, encounter_ref: str, day_2_hyperglycemia: int) -> Observation:
    ...
    observation.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/hyperglycemia-observation-profile"])
    observation.category = [CodeableConcept(coding=[Coding(system="http://terminology.hl7.org/CodeSystem/observation-category", code="exam", display="Exam")])]
    return observation
...
def build_observation_ge10(patient_ref: str, encounter_ref: str, ge10: bool) -> Observation:
    ...
    observation.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/glucose-ge10-observation-profile"])
    observation.category = [CodeableConcept(coding=[Coding(system="http://terminology.hl7.org/CodeSystem/observation-category", code="exam", display="Exam")])]
    return observation
...
def build_observation_highest_hyperglycemia_value(patient_ref: str, encounter_ref: str, observation_measurement_ref: list[str] | None, highest_hyperglycemia_value: int | str| None ) -> Observation:
    ...
    observation.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/highest-hyperglycemia-value-observation-profile"])
    observation.category = [CodeableConcept(coding=[Coding(system="http://terminology.hl7.org/CodeSystem/observation-category", code="exam", display="Exam")])]
