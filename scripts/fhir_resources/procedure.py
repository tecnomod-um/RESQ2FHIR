...
from fhir.resources.fhirtypes import MetaType, QuantityType, RangeType
from fhir.resources.codeableconcept import CodeableConcept
from fhir.resources.codeablereference import CodeableReference
from fhir.resources.extension import Extension
from fhir.resources.period import Period
from enum_models import (
    AssessmentContext, IchTreatment, ImagingDone, ImagingType, PostAcuteCare, PostStrokeProcedures, ProcedureNotDoneReason,SwallowingScreeningType,
...
def build_imaging_procedure(patient_ref: str, encounter_ref: str, diagnostic_report_ref: str, imaging_done: str | None, imaging_type: ImagingType | None, post_acute_care: bool, imaging_timestamp: str | None) -> Procedure:
    """
    ...
    if imaging_done is not None:
        imaging_done_concept = ImagingDone.by_id(imaging_done)
        
        if imaging_done_concept.id == ImagingDone.YES.id:
            status_reason = None
            procedure_final.status = "completed"
        else:
            if imaging_done_concept.id == ImagingDone.NO.id:
                status_reason = ProcedureNotDoneReason.UNKNOWN
                procedure_final.status = "not-done"
            elif imaging_done_concept.id == ImagingDone.ELSEWHERE.id:
...
def build_carotid_imaging_procedure(patient_ref: str, encounter_ref: str, diagnostic_report_ref: str, carotid_arteries_imaging_done: bool) -> Procedure:
    ...
    procedure.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/stroke-carotid-imaging-procedure-profile"])
...
def build_endarterectomy_procedure(patient_ref: str, encounter_ref: str, diagnostic_report_ref: str, carotid_endarterectomy: bool | None, carotid_endarterectomy_timing: str | None) -> Procedure:
    """
    ...
    procedure = Procedure(
        ...
        encounter=Reference(reference=encounter_ref),
        status="completed",
        meta=MetaType(profile = ["http://tecnomod-um.org/StructureDefinition/stroke-carotid-endarterectomy-procedure-profile"]),
        report = [Reference(reference=diagnostic_report_ref)])
    ...
    if carotid_endarterectomy is True:
        ...
        if carotid_endarterectomy_timing is not None:
            if carotid_endarterectomy_timing == "in 24 hours":
                procedure.occurrenceRange = RangeType(high=QuantityType(value=Decimal(24), unit="h", system="http://unitsofmeasure.org", code="h", comparator="<="))
            elif carotid_endarterectomy_timing == "24 hours to 2 weeks":
                procedure.occurrenceRange = RangeType(low=QuantityType(value=Decimal(24), unit="h", system="http://unitsofmeasure.org", code="h", comparator=">"), high=QuantityType(value=Decimal(2), unit="wk", system="http://unitsofmeasure.org", code="wk", comparator="<="))
            elif carotid_endarterectomy_timing == "after 2 weeks":
                procedure.occurrenceRange = RangeType(low=QuantityType(value=Decimal(2), unit="wk", system="http://unitsofmeasure.org", code="wk", comparator=">"))
...
def build_swallowing_screening_procedure(patient_ref: str, encounter_ref: str, practitioner_role_ref: str, swallowing_screening_done: bool | None = False, swallowing_screening_type: SwallowingScreeningType | None = None, swallowing_screening_timing: SwallowingScreeningTiming | None = None) -> Procedure:
    ...
    procedure.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/stroke-swallow-procedure-profile"])
...
def build_thrombolysis_procedure(patient_ref: str, encounter_ref: str, condition_ref: str, location_ref: str | None = None, thrombolysis: bool | None = None, post_acute_care: bool = False, no_thrombolysis_reason_id: str | None = None, bolus_timestamp:str|None = None) -> Procedure:
    """
    ...
    procedure.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/stroke-mechanical-procedure-profile"])
    ...
    if thrombolysis is None and no_thrombolysis_reason_id is None:
        ...
        if no_thrombolysis_reason_id is not None:
            no_thrombolysis_reason_coding = ProcedureNotDoneReason.by_id(no_thrombolysis_reason_id).to_coding()
            code_reason = CodeableConcept(coding=[no_thrombolysis_reason_coding])
            procedure.status = "not-done"
            procedure.statusReason = code_reason
            return procedure
        else:
            no_thrombolysis_reason = ProcedureNotDoneReason.UNKNOWN.to_coding()
            code_reason = CodeableConcept(coding=[no_thrombolysis_reason])
            procedure.status = "not-done"
            procedure.statusReason = code_reason
            return procedure
    else:
...
def build_thrombectomy_procedure(thrombectomy: bool, post_acute_care: bool, patient_ref: str, encounter_ref: str, condition_ref: str, no_thrombectomy_reason_id = None, reperfusion_timestamp = None, puncture_timestamp = None, mt_complications_perforation: bool = False, mt_complications_dissection: bool = False, mt_complications_hematoma: bool = False, mt_complications_embolism: bool = False, mt_complications_other: bool = False) -> Procedure:
    """
    ...
    if thrombectomy is False or thrombectomy is None :
        if no_thrombectomy_reason_id is True or no_thrombectomy_reason_id is None:
            ...
            no_thrombectomy_reason_coding = ProcedureNotDoneReason.by_id(no_thrombectomy_reason_id).to_coding()
...
# def build_post_imaging_procedure(patient_ref: str, encounter_ref: str, post_neurosurgery_imaging_done: bool)
