...
from fhir.resources.fhirtypes import CodeableReferenceType, MetaType 
from fhir.resources.reference import Reference
from fhir.resources.coding import Coding
from fhir.resources.codeableconcept import CodeableConcept
from fhir.resources.extension import Extension
from enum_models import AnaliticsCodes, BleedingReason, BodySites, ConceptEnum, DiagnosisStrokes, PostStrokeComplications, RiskFactor, SpecificFinding, StrokeEtiology, StrokeEtiology, StrokeType, ClinicalStatusCodes, Symptomps
from utils import parse_datetime


def build_stroke_diagnosis_condition_profile(patient_ref: str, encounter_ref: str, stroke_type: StrokeType | None, stroke_type_mimics: DiagnosisStrokes | None, stroke_etiology_known: bool, stroke_etiology: StrokeEtiology | None , bleeding_reason_found: bool, bleeding_reason: BleedingReason | None, wakeup_stroke: bool, sleep_timestamp: str | None, onset_timestamp: str | None, obs_symptoms_tia_ref: str | None, ich_infratentorial: bool, ich_supratentorial: bool, intraventricular_hemorrhage: bool, subarachnoid_hemorrhage: bool) -> Condition:
    """
    ...
    if stroke_type == StrokeType.TRANSIENT_ISCHEMIC: 
        if obs_symptoms_tia_ref is not None:
            condition.evidence = [CodeableReferenceType(reference=Reference(reference=obs_symptoms_tia_ref))]
    ...
    condition.meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/stroke-diagnosis-condition-profile"])
    extension_list = []
    ...
    for status_key, rf in risk_factors:
        ...
        for r in rf:
            ...
            condition = Condition(
                subject=Reference(reference=patient_ref),
                code=code_rf,
                meta=MetaType(profile=["http://tecnomod-um.org/StructureDefinition/stroke-risk-factor-condition-profile"]),
...
def build_stenosis_condition(patient_ref:str, encounter_ref:str, carotid_stenosis:bool, carotid_stenosis_level: ConceptEnum) -> Condition:
    """
    Build a FHIR Condition resource for carotid stenosis.
    
    Args:
        patient_ref: Reference to the Patient resource
        encounter_ref: Reference to the Encounter resource
        procedure_ref: Reference to the Procedure resource for carotid imaging
        carotid_stenosis: Boolean indicating presence of carotid stenosis
        carotid_stenosis_level: ConceptEnum representing the level of carotid stenosis (if present)
    Returns:
        Condition resource with carotid stenosis diagnosis
    """

    stenosis_condition = Condition(
        subject=Reference(reference=patient_ref),
        encounter=Reference(reference=encounter_ref),
        code = CodeableConcept(coding=[SpecificFinding.CAROTID_STENOSIS.to_coding()])
    )

    if carotid_stenosis is True:
        stenosis_condition.clinicalStatus = CodeableConcept(coding=[ClinicalStatusCodes.ACTIVE.to_coding()])
        if carotid_stenosis_level is "bellow 50":
            stenosis_condition.severity = CodeableConcept(coding=[QualifierValue.MILD.to_coding()])
        elif carotid_stenosis_level is "50 to 70":
            stenosis_condition.severity = CodeableConcept(coding=[QualifierValue.MODERATE.to_coding()])
        elif carotid_stenosis_level is "over 70":
            stenosis_condition.severity = CodeableConcept(coding=[QualifierValue.SEVERE.to_coding()])
        elif carotid_stenosis_level is "occlusion":
            stenosis_condition.severity = CodeableConcept(coding=[QualifierValue.OCCLUSION.to_coding()])
    else:
        stenosis_condition.clinicalStatus = CodeableConcept(coding=[ClinicalStatusCodes.INACTIVE.to_coding()])

    return stenosis_condition
    ...
    if post_stroke_any is True:  
        if post_stroke_sepsis is True:
            sepsis_condition = Condition(
                ...
                meta = MetaType(profile=["http://tecnomod-um.org/StructureDefinition/post-stroke-complication-condition-profile"])
            )
