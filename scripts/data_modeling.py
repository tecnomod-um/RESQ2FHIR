...
from scripts.enum_models import AppointmentSituation, ArrivalMode, AssessmentContext, AtrialFibrillationOrFlutter, BleedingReason, DiagnosisStrokes, DischargeDestination, DischargeFacilityDepartment, HospitalizedIn, ImagingDone, ImagingType, Locations, MRsScore, MTiciScore, Medications, NotMedicationReason, PostAcuteCare, ProcedureNotDoneReason, StrokeEtiology, StrokeType, ThreeMonthContactMode
from scripts.fhir_resources.appointment import build_follow_up_appointment
from scripts.fhir_resources.diagnosticReport import build_carotid_arteries_imaging_diagnostic_report, build_imaging_diagnostic_report, build_mechanical_thrombectomy_diagnostic_report
from scripts.fhir_resources.location import build_location
from scripts.fhir_resources.medicationAdministration import build_medicationAdministration
...
########################## 0. Admission Data ##########################
    ...
    ########################## 0.1. Organization ##########################
    hospital_name = safe_get(raw, "hospital_name", required=True)
    ...
    patient = build_Patient(patient_id=file_id, patient_sex=sex) # Build Patient resource using file_id and sex
    ...
    encounter = build_stroke_encounter_profile(patient_ref=patient_ref, # Build Encounter resource using patient_ref and other relevant data from raw
                                                arrival_mode=ArrivalMode.by_id(safe_get(raw, "arrival_mode_id", required=False)), # Obtain arrival mode from raw data and convert to ArrivalMode enum, field not required
                                                ...
                                                admission_department=Locations.by_id(safe_get(raw, "admission_department", required=False)), # Obtain admission department from raw data and convert to Locations enum, field not required
                                                first_contact_place=Locations.by_id(safe_get(raw, "first_contact_place", required=False)), # Obtain first contact place from raw data and convert to Locations enum, field not required
                                                ...
                                                hospitalized_in=HospitalizedIn.by_id(safe_get(raw, "hospitalized_in", required=True)), # Obtain hospitalized in from raw data and convert to HospitalizedIn enum, field required
...
############################# 1. Baseline Data ##########################################
    ...
    ################### 1.7. mRS score #############################
    prestroke_mrs = safe_get(raw, "prestroke_mrs", required=True) # Check if prestroke mRS score is present in raw data, not required
    ...
    if inr_mode is not None:
        inr_value = safe_get(raw, "inr_value", required=True) # If INR model is present, check if inr_value is present in raw data, required if inr_mode is present
...
####################################### 2. Imaging, diagnosis and treatment ########################################################
    ...
    ###################################### 2.1 Brain imaging ######################################
    imaging_type = safe_get(raw, "imaging_type", required=True)
    ...
    if stroke_type is not None:
        ...
        condition_stroke = build_stroke_diagnosis_condition_profile(patient_ref = patient_ref, # Build Condition resource for stroke diagnosis using patient_ref and other relevant data from raw
                                                                    ...
                                                                    stroke_type_mimics=DiagnosisStrokes.by_id(safe_get(raw, "stroke_mimics_diagnosis", required=False)), # Obtain stroke type mimics from raw data and convert to ConceptEnum, field not required
        ...
        ############################ 2.2.2.4 Thombolysis administration reversal (MedicationAdministration) ######################################
            ...
            procedure_thrombolysis = build_thrombolysis_procedure(patient_ref=patient_ref,
                                                                    ...
                                                                    post_acute_care=False,
                                                                    no_thrombolysis_reason_id=no_thrombolysis_reason_id,
        ...
        thrombectomy_done = safe_get_bool(raw, "thrombectomy", required=True) # Check if mechanical thrombectomy done boolean is present in raw data, not required
        if thrombectomy_done is not None and thrombectomy_done is False:
            no_thrombectomy_reason_id = safe_get(raw, "no_thrombectomy_reason", required=True) # Check if no mechanical thrombectomy reason is present in raw data, not required
            procedure_thrombectomy = build_thrombectomy_procedure(patient_ref=patient_ref,
                                                                encounter_ref=encounter_ref,
                                                                condition_ref=condition_stroke_ref,
                                                                thrombectomy=thrombectomy_done,
                                                                post_acute_care=False,
                                                                no_thrombectomy_reason_id=no_thrombectomy_reason_id) # Build Procedure resource for mechanical thrombectomy using patient_ref, encounter_ref and stroke diagnosis reference, field not required
        ...
        if stroke_type == StrokeType.SUBARACHNOID_HEMORRHAGE:
            ...
            if nimodipine_administration is not None and nimodipine_administration_timing is not None:
                medicationAdministration_nimodipine = build_medicationAdministration(patient_ref=patient_ref,
                                                                                encounter_ref=encounter_ref,
                                                                                condition_ref=condition_stroke_ref,
                                                                                ...
                                                                                medication= Medications.NIMODIPINE, # Use a specific medication for nimodipine, can be extended to use different medications if needed
                                                                                dose = None, # Dose is not provided for nimodipine in raw data, pass None
...
##################################### 3. Post-acute care data #####################################
    ...
    if post_acute_care is not None and post_acute_care:
        ...
        atrial_fibrillation_or_flutter = safe_get(raw, "atrial_fibrillation_or_flutter", required=True) # Check if atrial fibrillation or flutter boolean is present in raw data, not required 
        if atrial_fibrillation_or_flutter is not None:
            observation_af_or_f =build_observation_Af_or_F(patient_ref=patient_ref,
                                                        encounter_ref=encounter_ref,
                                                        atrial_fibrillation_or_flutter=AtrialFibrillationOrFlutter.by_id(atrial_fibrillation_or_flutter)) # Build Observation resource for atrial fibrillation or flutter using patient_ref, encounter_ref and atrial fibrillation or flutter boolean from raw data, field not required
        ...
        if carotid_arteries_imaging:
            ...
            carotid_stenosis_observation = build_observation_carotid_stenosis(patient_ref=patient_ref,
                                                                            ...
                                                                            carotid_stenosis_level=carotid_stenosis_level) # Build Observation resource for carotid stenosis using patient_ref, encounter_ref and carotid stenosis percentage and level from raw data, field not required
        ...
        fever_observation = build_observation_fever(patient_ref=patient_ref,
                                                ...
                                                fever=safe_get_bool(raw, "fever_diagnosed", required=True)
        ...
        if paracetamol_on_fever is not None:
            ...
            medicationAdministration_paracetamol = build_medicationAdministration(patient_ref=patient_ref,
                                                                encounter_ref=encounter_ref,
                                                                condition_ref=None, # No condition reference for paracetamol administration for fever, can be extended to reference a specific condition if needed
                                                                procedure_ref=None, # No procedure reference for paracetamol administration for fever, can be extended to reference a specific procedure if needed
                                                                observation_ref=fever_observation_ref, # Reference to the fever observation
                                                                medication= Medications.PARACETAMOL, # Use a specific medication for paracetamol, can be extended to use different medications if needed
                                                                dose = None, # Dose is not provided for paracetamol in raw data, pass None
                                                                medication_timestamp=paracetamol_on_fever_timing) # Pass paracetamol administration timestamp to build_medicationAdministration
        ...
        highest_hyperglycemia_observation = build_observation_highest_hyperglycemia_value(patient_ref=patient_ref,
                                                        encounter_ref=encounter_ref,
                                                        observation_measurement_ref= obs_ref_list,
                                                        highest_hyperglycemia_value=safe_get(raw, "highest_glucose_within48h", required=True)
        ...
        if insulin_bool is not None:
            ...
            insulin_on_hyperglycemia_medicationAdministration = build_medicationAdministration(patient_ref=patient_ref,
                                                                encounter_ref=encounter_ref,
                                                                condition_ref=None, # No condition reference for insulin administration for hyperglycemia, can be extended to reference a specific condition if needed
                                                                procedure_ref=None, # No procedure reference for insulin administration for hyperglycemia, can be
                                                                observation_ref=highest_hyperglycemia_observation_ref, # Reference to the highest hyperglycemia observation
                                                                medication= Medications.INSULIN, # Use a specific medication for insulin, can be extended to use different medications if needed
                                                                dose = None, # Dose is not provided for insulin in raw data, pass None
                                                                medication_timestamp=medicationAdministration_insulin_timing) # Pass insulin administration timestamp to build_medicationAdministration
...
################################# 4.Discharge Data ############################################### 
    ...
    discharge_any_med = safe_get_bool(raw, "discharge_any_medication", required=True) # Check if any medication at discharge boolean is present in raw data, not required
    ...
    if stroke_management_appointment is not None:
        stroke_appointment = build_follow_up_appointment(patient_ref=patient_ref,
                                                        management_appointment= AppointmentSituation.by_id(stroke_management_appointment)) # Build Appointment resource for follow-up appointment using patient_ref, encounter_ref and follow-up appointment boolean from raw data, field not required
...
################################# 6. Timing specific metrics (KPis) ######################################
    ...
    if door_to_ich_evacuation is not None:
    ...
    if door_to_imaging is not None:
    ...
    if iv_antihypertensive is not None:
        ...
        medicationAdministration_iv_antihypertensive = build_medicationAdministration(patient_ref=patient_ref,
                                                                ...
                                                                observation_ref= observation_vital_signs_ref, # No observation reference for IV antihypertensive administration, can be extended to reference a specific observation if needed
    ...
    if door_to_iv_antihypertensive is not None:
